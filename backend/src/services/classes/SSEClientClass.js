...
class SSEClientClass extends ISSEClient {
  constructor(storage, eventEmitter, logger, features) {
    ...
    this.config = {
      ...
      reconnectInterval: parseInt(process.env.SSE_RECONNECT_INTERVAL, 10) || 5000,
      maxReconnectAttempts: parseInt(process.env.SSE_MAX_RECONNECT_ATTEMPTS, 10) || 3,
  ...
  connectToAuction(productId, auctionId) {
    if (!this.config.enabled) {
  ...
  handleBidUpdate(productId, auctionId, bidData) {
    const updateData = {
  ...
  handleAuctionClosed(productId, auctionId, closeData) {
    const updateData = {
      status: 'closed',
      isClosed: true,
  ...
  handleReconnection(productId, auctionId) {
