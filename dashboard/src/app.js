...
class AuctionMonitorUI {
  ...
  getBackendUrl() {
    ...
    if (typeof window.settingsManager !== 'undefined' && window.settingsManager && window.settingsManager.getBackendUrl) {
      return window.settingsManager.getBackendUrl();
    }
    ...
    this.ws.onopen = () => {
      ...
      if (!authToken) {
        ...
        if (window.location.hostname === 'localhost') {
          ...
          authToken = window.prompt('Please enter your authentication token:'); // eslint-disable-line no-alert
          if (authToken) {
            localStorage.setItem('authToken', authToken);
          } else {
    ...
    switch (data.type) {
      ...
      case 'authenticated':
        if (data.success) {
          ...
          Logger.error('Authentication failed. Please refresh the page and enter a valid token.');
          this.ws.close();
        }
        break;
  ...
  updateMaxBid(auctionId, value) {
    const auction = this.auctions.get(auctionId);
    const maxBid = parseInt(value, 10) || 0;
    ...
    if (auction && auction.data && auction.data.nextBid) {
      ...
      if (maxBid > 0 && maxBid < minimumBid) {
        ...
        Logger.error(`Maximum bid must be at least $${minimumBid} (the minimum next bid)`);
  ...
  patchAuctionCard(auction) {
    ...
    if (isWinning) { card.classList.add('winning'); } else if (timeRemaining <= 30 && timeRemaining > 0) { card.classList.add('urgent'); } else if (timeRemaining <= 300 && timeRemaining > 0) { card.classList.add('warning'); }
    ...
    if (auction.config) {
      ...
      if (strategyContainer) {
        ...
        buttons.forEach(button => {
          ...
          let strategy = '';
          if (onclickAttr && onclickAttr.includes("'increment'")) { strategy = 'increment'; } else if (onclickAttr && onclickAttr.includes("'sniping'")) { strategy = 'sniping'; }
      ...
      if (maxBidInput) {
        ...
        if (parseInt(maxBidInput.value, 10) !== auction.config.maxBid) {
        ...
        if (auction.data && auction.data.nextBid) {
          ...
          if (maxBidContainer) {
            const currentValue = parseInt(maxBidInput.value, 10) || 0;
            // Don't show error if user is winning, even if max bid is less than next bid
  ...
  createAuctionCard(auction) {
    ...
    card.addEventListener('click', (_e) => {
      ...
      if (_e.target.tagName === 'BUTTON' || _e.target.tagName === 'INPUT' || _e.target.closest('button') || _e.target.closest('input')) {
        return;
      }
  ...
  initModalHandlers() {
    ...
    document.addEventListener('keydown', (_e) => {
      if (_e.key === 'Escape' && modal.classList.contains('show')) {
        this.closeBidHistory();
      }
    });
  }
