...
(function () {
  ...
  Logger.info('Hot Deals script loading...');
  ...
  class HotDealsManager {
    ...
    init() {
      ...
      if (this.elements.discountSlider) {
        this.elements.discountSlider.addEventListener('input', (_e) => {
      ...
      if (this.elements.maxBidPercentageSlider) {
        this.elements.maxBidPercentageSlider.addEventListener('input', (_e) => {
          const value = parseInt(_e.target.value, 10);
      ...
      if (this.elements.maxPriceInput) {
        this.elements.maxPriceInput.addEventListener('keypress', (_e) => {
          if (_e.key === 'Enter') {
            this.applyFilters();
          }
        });
      ...
      if (this.elements.searchInput) {
        this.elements.searchInput.addEventListener('keypress', (_e) => {
          if (_e.key === 'Enter') {
            this.applyFilters();
          }
        });
    ...
    applyFilters() {
      ...
      const discountValue = parseInt(this.elements.discountSlider.value, 10);
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex gap-2 mt-3">
                        ...
                        <a href="${deal.auctionUrl || '#'}" target="_blank" rel="noopener noreferrer" 
                           ...
                           onclick="if(!this.href || this.href === '#' || this.href.includes('undefined')) { event.preventDefault(); Logger.error('Deal URL not available'); return false; }">
                            View Deal
                        </a>
                    </div>
                </div>
    ...
    setupMonitorModalHandlers() {
      ...
      confirmBtn.addEventListener('click', async () => {
        ...
        if (!maxBid || maxBid <= 0) {
          Logger.error('Please enter a valid maximum bid amount');
          return;
        }
      ...
      document.getElementById('monitor-max-bid').addEventListener('keypress', (_e) => {
        if (_e.key === 'Enter') {
          confirmBtn.click();
        }
      });
  ...
  Logger.info('Setting up Hot Deals initialization...');
  if (document.readyState === 'loading') {
    Logger.info('Document still loading, waiting for DOMContentLoaded...');
    document.addEventListener('DOMContentLoaded', () => {
      Logger.info('DOMContentLoaded fired, creating HotDealsManager...');
      window.hotDealsManager = new HotDealsManager();
    });
  } else {
    Logger.info('Document ready, creating HotDealsManager immediately...');
    window.hotDealsManager = new HotDealsManager();
  }
}());