...
function handleNavigation() {
  console.log('NAH: Navigation detected, reinitializing...');
...
function detectSingleAuction() {
  const auctionId = extractAuctionIdFromUrl();
  if (auctionId) {
    console.log('NAH: Detected auction ID:', auctionId);
    
    // Check if this auction is already being monitored
    chrome.runtime.sendMessage({
      ...
      const isMonitored = response && response.auctions && 
        response.auctions.some(auction => auction.id === auctionId);
      
      if (isMonitored) {
        console.log('NAH: Auction is already being monitored');
        monitoredAuctions.add(auctionId);
        // Update display for already monitored auction
        const monitoredAuction = response.auctions.find(auction => auction.id === auctionId);
        if (monitoredAuction) {
          // Update display after panel is created
          setTimeout(() => {
            updateAuctionDisplay(auctionId, monitoredAuction);
            updateMonitoringStatusInAllShadowRoots(auctionId, 'Monitoring active');
          }, 100);
        }
      }
      
      // Find a suitable container for our panel
      let targetContainer = null;
      
      // Strategy 1: Find the bid button's container (PLACE BID, RAISE BID, or LOGIN TO BID)
      const bidButton = Array.from(document.querySelectorAll('button')).find(
        btn => {
      ...
      if (bidButton) {
        targetContainer = bidButton.closest('div');
        // Go up to find a larger container
        while (targetContainer && targetContainer.children.length < 3) {
      ...
      // Strategy 2: Try to find the bidding area by looking for the minimum bid input
      if (!targetContainer) {
        const minBidInput = document.querySelector('input[type="number"][aria-label*="Minimum bid"]');
        if (minBidInput) {
          targetContainer = minBidInput.closest('div');
          // Go up a few levels to find the right container
          for (let i = 0; i < 3 && targetContainer; i++) {
            const parent = targetContainer.parentElement;
            if (parent.tagName === 'MAIN' || parent.tagName === 'BODY') break;
            targetContainer = parent;
          }
      ...
      if (targetContainer) {
        console.log('NAH: Adding monitoring panel with Shadow DOM');
        addDetailedMonitoringWithShadow(targetContainer, auctionId, isMonitored);
        addFloatingMonitorButtonWithShadow(auctionId, isMonitored);
      }
    });
  }
}
