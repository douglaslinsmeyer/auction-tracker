...
Then('each auction should have its own polling timer', async function() {
...
Then('CPU usage should be lower than legacy polling', async function() {
...
When('the polling queue processes updates', async function() {
  ...
  if (originalUpdate) {
    const trackingUpdate = async (auctionId) => {
...
Given('I am monitoring {int} auctions in the queue', async function(count) {
  await this.steps.call(this, `I start monitoring ${count} auctions`);
});
...
Then('the auction should be removed from the queue', async function() {
  const auctionMonitor = require('../../../src/services/auctionMonitor');
...
Then('the queue should handle all auctions', async function() {
  const auctionMonitor = require('../../../src/services/auctionMonitor');
...
Then('no more than {int} API requests per second should be made', async function(maxRequests) {
...
Then('failed polls should be rescheduled', async function() {
  const auctionMonitor = require('../../../src/services/auctionMonitor');
