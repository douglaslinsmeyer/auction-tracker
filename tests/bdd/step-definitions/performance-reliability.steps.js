...
When('I send {int} messages within {int} seconds', async function(count, seconds) {
  const messages = [];
  const startTime = Date.now();
...
When('I make {int} monitoring requests within {int} seconds', async function(count, seconds) {
...
When('I make {int} status check requests within {int} seconds', async function(count, seconds) {
  this.statusRequests = [];
...
When('all auctions update simultaneously', function() {
  ...
  queueState.auctions.forEach(auction => {
...
When('average response time exceeds {int}ms for {int} minute', function(ms, minutes) {
...
Then('subsequent requests should return {int} {string}', function(status, message) {
  const failures = this.apiRequests.filter(r => r.status === status);
  expect(failures.length).to.be.greaterThan(0);
});

Then('the response should include a {string} header', function(header) {
...
Then('allow {int} test request through', function(count) {
  expect(circuitBreakerState).to.equal('HALF_OPEN');
});
...
Then('high-priority auctions should be checked every {int} seconds', function(seconds) {
  const highPriority = Array.from(queueState.auctions.values())
    .filter(a => a.priority === 'high');
  expect(highPriority.length).to.be.greaterThan(0);
});

Then('low-priority auctions should be checked every {int} seconds', function(seconds) {
...
Then('CPU usage should not exceed {int}%', function(percent) {
...
Then('attempt to reconnect every {int} seconds', function(seconds) {
