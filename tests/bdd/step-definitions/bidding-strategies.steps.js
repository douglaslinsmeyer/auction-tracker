...
Before(function() {
  ...
  bidPlacementStub = sinon.stub(nellisApi, 'placeBid').callsFake(async (auctionId, amount) => {
...
Given('the following auctions exist:', function(dataTable) {
  ...
  auctions.forEach(auctionData => {
    testAuctions[auctionData.id] = AuctionFactory.create({
      id: auctionData.id,
      currentBid: parseInt(auctionData.currentBid),
      timeLeft: parseInt(auctionData.timeLeft),
...
Then('the system should log {string}', function(logMessage) {
...
Then('a retry should be attempted after {int} seconds', function(seconds) {
